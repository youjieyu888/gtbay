...
<?php
if (!isset($_SESSION)) {
    session_start(); //login only if not login before
}
...
ini_set('display_errors', '1'); //don't display error to users
ini_set("log_errors", 'on');  // but log them
ini_set("error_log", getcwd() . SEPARATOR ."error.log"); //specify the file to log error

define('NEWLINE',  '<br>' );
define('REFRESH_TIME', 'Refresh: 1; ');

$encodedStr = basename($_SERVER['REQUEST_URI']); //returns the file name from the path
//convert '%40' to '@'  example: request_friend.php?friendemail=pam@dundermifflin.com
$current_filename = urldecode($encodedStr); //in case decoded??
	
if($showQueries){
    array_push($query_msg, "<b>Current filename: ". $current_filename . "</b>"); 
}//append, query_msg changes

define('DB_HOST', "localhost");
define('DB_PORT', "3306");
define('DB_USER', "gatechUser");
define('DB_PASS', "gatech123");
define('DB_SCHEMA', "cs6400_sp18_team071");

$db = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_SCHEMA, DB_PORT);
//connect to sql improved
